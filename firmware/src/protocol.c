...
typedef struct {
  ...
  uint8_t pulse_duration;          // duration of each pulse (in stepper IRQs, 31.875us)
  ...
  switch(command) {
    ...
    case CMD_LINE:
      planner_line( st.target[X_AXIS], st.target[Y_AXIS], st.target[Z_AXIS],
                    st.feedrate, st.pulses_per_mm, st.pulse_duration, st.raster_bytes);
      break;
    case CMD_DWELL:
      //planner_dwell(st.pierce_duration, st.intensity);
      break;
